...
function NewsItem({ offset, n }) {
  ...
  const [currentStyle, setCurrentStyle] = useState({}); // Store the current style

  useEffect(() => {
    const apiKey = "cc68e01925fa4ec6af57312ea47948b5";
    const endpoint = `https://newsapi.org/v2/everything?q=fashion&apiKey=${apiKey}`;
    ...
    fetch(endpoint)
      ...
      .then((data) => {
        if (data.status === "ok") {
          ...
          const styleObject = liStyles.find((style) => style.offset === offset);
      ...
      <ul className="news-item-box">
        {articles.map((article, index) => (
          <li
            key={article.title}
            style={{
              ...currentStyle, // Use the current style for the li element
              ...{
                textAlign: "center",
                fontSize: "0.5em",
                display: "flex",
          ...
          >
          <div className="news-object" style={{
                ...customStyles[(index + offset -5) % customStyles.length]}}
          >
